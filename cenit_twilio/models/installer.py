...
DATA = {
  "name": "MyTwilio API Integration_",
  "shared_version": "1.0.1",
  "description": "My Twilio API Integration",
  "data": {
    "name": "My Twilio API Integration",
    "flows": [
      {
...
class CollectionInstaller(models.TransientModel):
    ...
    def install_collection(self):
        ...
        path = "/api/v1/push"
        data = {
            'shared_collection': DATA
        }

        rc = cenit_api.post(path, data)
        {'success': {'shared_collections': [{'id': '557898596461724df5020000'}]}}
        sharedID = rc.get(
            'success', {}
        ).get(
            'shared_collections', [{}]
