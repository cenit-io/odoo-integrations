...
COLLECTION_NAME = "clickmeter_v2"
COLLECTION_VERSION = "0.1"
COLLECTION_PARAMS = {
    "On connection 'ClickMeter v2 Connection' template parameter 'api_key'":'api_key',
}

class CenitIntegrationSettings(models.TransientModel):
    _name = "cenit.clickmeter_v2.settings"
    ...
    def get_default_api_key(self, cr, uid, ids, context=None):
        api_key = self.pool.get('ir.config_parameter').get_param(
            cr, uid, 'odoo_cenit.clickmeter_v2.api_key', default=None, context=context
        )
    ...
    def set_api_key(self, cr, uid, ids, context=None):
        ...
        for record in self.browse(cr, uid, ids, context=context):
            config_parameters.set_param (
                cr, uid, 'odoo_cenit.clickmeter_v2.api_key', record.api_key or '',
                context=context
            )
