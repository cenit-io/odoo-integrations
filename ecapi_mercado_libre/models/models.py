...
class OmnaIntegration(models.Model):
    ...
    def _get_integrations_channel_selection(self):
        try:
            ...
            selection = []
            for channel in list(filter(lambda d: d['group'] == 'MercadoLibre', response.get('data'))):
    ...
    def _get_logo(self, channel):
        ...
        logo = ''
        if 'MercadoLibre' in channel:
            logo = modules.get_module_resource('ecapi_mercado_libre', 'static/src/img', 'mercadolibre_logo.png')
        else:
            logo = modules.get_module_resource('ecapi_mercado_libre', 'static/src/img', 'marketplace_placeholder.png')
...
class ProductTemplate(models.Model):
    ...
    def unlink(self):
        ...
        #     record_id = self.env.ref('ecapi_mercado_libre.delete_unlinked_products_cron').id
...
class ProductProduct(models.Model):
    ...
    def unlink(self):
        ...
        #         except Exception as e:
        #             _logger.error(e)
        #     record_id = self.env.ref('ecapi_mercado_libre.delete_unlinked_variants_cron').id
        #     data = {'active': True,
    ...
    def launch_wizard_list(self):
        view_id = self.env.ref('ecapi_mercado_libre.view_properties_values_wizard').id
        context = dict(
            self.env.context,
            integration_id=self.integration_ids.id,
...
class OmnaIntegrationChannel(models.Model):
    ...
    def _get_logo(self, group):
        logo = ''
        if group == 'MercadoLibre':
            logo = '/ecapi_mercado_libre/static/src/img/mercadolibre_logo.png'
        else:
            logo = '/ecapi_mercado_libre/static/src/img/marketplace_placeholder.jpg'
        return logo
...
class AccountMoveLine(models.Model):
    ...
    integration_id = fields.Many2one(string='Integration', related='move_id.integration_id')
...
class SaleCustom(models.Model):
   ...
   def get_sale_order(self):
       ret_list = []
       req = ("SELECT sale_order.name, rp.name AS customer, sale_order.amount_total, sale_order.state "
                   "FROM sale_order "
