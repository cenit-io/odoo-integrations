...
class ResUsers(models.Model):
    ...
    def _compute_omna_manager(self):
        self.context_omna_manager = self.has_group('ecapi_mercado_libre.group_omna_manager')

    def _default_current_tenant(self):
        return self.env['omna.tenant'].search([], limit=1).id

    def _default_omna_urls(self):
        url = self.env["ir.config_parameter"].sudo().get_param("ecapi_mercado_libre.cenit_url")
