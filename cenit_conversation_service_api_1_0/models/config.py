...
COLLECTION_NAME = "conversation_service_api_1_0"
...
class CenitIntegrationSettings(models.TransientModel):
    _name = "cenit.conversation_service_api_1_0.settings"
    _inherit = 'res.config.settings'

    ############################################################################
    # Pull Parameters
    ############################################################################
    host = fields.Char('Conversation host')

    ############################################################################
    # Default Getters
    ############################################################################
    def get_default_host(self, cr, uid, ids, context=None):
        host = self.pool.get('ir.config_parameter').get_param(
            cr, uid, 'odoo_cenit.conversation_service_api_1_0.host', default=None, context=context
        )
        return {'host': host or ''}
    ...
    def set_host(self, cr, uid, ids, context=None):
        ...
        for record in self.browse(cr, uid, ids, context=context):
            config_parameters.set_param (
                cr, uid, 'odoo_cenit.conversation_service_api_1_0.host', record.host or '',
                context=context
            )
