...
COLLECTION_NAME = "recipe_and_grocery_list_api_v_partner"
COLLECTION_VERSION = "0.1"
COLLECTION_PARAMS = {
    "On connection 'Recipe and Grocery List API v partner Connection' template parameter 'api_key'":'api_key',
}

class CenitIntegrationSettings(models.TransientModel):
    _name = "cenit.recipe_and_grocery_list_api_v_partner.settings"
    ...
    def get_default_api_key(self, cr, uid, ids, context=None):
        api_key = self.pool.get('ir.config_parameter').get_param(
            cr, uid, 'odoo_cenit.recipe_and_grocery_list_api_v_partner.api_key', default=None, context=context
        )
    ...
    def set_api_key(self, cr, uid, ids, context=None):
        ...
        for record in self.browse(cr, uid, ids, context=context):
            config_parameters.set_param (
                cr, uid, 'odoo_cenit.recipe_and_grocery_list_api_v_partner.api_key', record.api_key or '',
                context=context
            )
