...
class Omna(http.Controller):
    ...
    def sing_in(self, code, **kw):
        ...
        action = request.env.ref('ecapi_lazada.action_omna_orders').read()[0]
    ...
    def authorize_integration(self, integration_id, **kw):
        ...
        if integration:
            ...
            # redirect = '/web#action=%s&model=omna.integration&view_type=kanban&menu_id=%s' % (request.env.ref('ecapi_lazada.action_omna_integration').id, request.env.ref('ecapi_lazada.menu_omna_my_integrations').id)
            # redirect = '/web#action=%s&cids=1&home=&menu_id=%s&model=omna.integration&view_type=kanban' % (request.env.ref('ecapi_lazada.action_omna_integration').id, request.env.ref('ecapi_lazada.menu_omna_my_integrations').id)
            # return werkzeug.utils.redirect(redirect)
            return werkzeug.utils.redirect('/web#action={0}'.format(request.env.ref('ecapi_lazada.action_omna_integration').id))
