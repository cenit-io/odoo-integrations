...
class CenitIntegrationSettings(models.TransientModel):
    ...
    def get_default_(self, cr, uid, ids, context=None):
         = self.pool.get('ir.config_parameter').get_param(
            cr, uid, 'odoo_cenit.350000plus_recipe_and_grocery_list_api_v2_partner.', default=None, context=context
        )
        return {'':  or ''}
    ...
    def set_(self, cr, uid, ids, context=None):
        ...
        for record in self.browse(cr, uid, ids, context=context):
            config_parameters.set_param (
                cr, uid, 'odoo_cenit.350000plus_recipe_and_grocery_list_api_v2_partner.', record. or '',
                context=context
            )
