...
class CenitIntegrationSettings(models.TransientModel):
    ...
    def get_default_username(self, context):
        username = self.env['ir.config_parameter'].get_param(
            'odoo_cenit.language_translator_api_2_0_0.username', default=None
        )
        return {'username': username or ''}

    def get_default_password(self, context):
        password = self.env['ir.config_parameter'].get_param(
            'odoo_cenit.language_translator_api_2_0_0.password', default=None
        )
    ...
    def set_username(self):
        ...
        for record in self.browse(self.ids):
            config_parameters.set_param (
                'odoo_cenit.language_translator_api_2_0_0.username', record.username or ''
            )
    ...
    def set_password(self):
        ...
        for record in self.browse(self.ids):
            config_parameters.set_param (
                'odoo_cenit.language_translator_api_2_0_0.password', record.password or ''
            )
